            ...
            {Object.keys(products).length === 0 ? (
                ...
                (
                    <div className="bg-white">
                        ...
                        <div className="bg-white">
                            <div className="max-w-2xl m-4 py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8 ">
                                <h2 className="sr-only">Products</h2>

                                <div className="grid grid-cols-1 gap-y-10 sm:grid-cols-2 gap-x-6 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
                                    {products.map((product, index) => (
                                        <Link to={`/product/${product.id}`} className="group  shadow-md p-2" key={index}>
                                            <div className="w-full h-64 aspect-w-1 aspect-h-1 bg-gray-200 rounded-lg overflow-hidden xl:aspect-w-7 xl:aspect-h-8">
                                                <img
                                                    src={product.image}
                                                    alt={product.title}
                                                    height="200px"
                                                    className="w-full bg-cover object-center object-cover group-hover:opacity-75"
                                                />
                                            </div>
